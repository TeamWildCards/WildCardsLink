...
class WildCardsMain:
    ...
    def SerialOpened()
        self.WildTray.set_serial_port(self.WildSerial.com_port)
    def SerialClosed()
        self.WildTray.set_serial_port(None)
...
def shutdownhandler(sig, frame):
    if MainProcess.runnning():
        print('\nExiting")
        task = asyncio.ensure_future(MainProcess.shutdown())
...
#    try:
        ...
        print("Set up server on port 9000")
